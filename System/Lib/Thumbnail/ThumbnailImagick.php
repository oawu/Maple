...
class ThumbnailImagick extends Thumbnail {
  ...
  public function pad($width, $height, $color = 'transparent') {
    ...
    if ($width <= 0 || $height <= 0)
      return Thumbnail::log('新尺寸錯誤！', '尺寸寬高一定要大於 0', '寬：' . $width, '高：' . $height);

    if ($width == $this->dimension[0] && $height == $this->dimension[1])
      return $this;

    if (!is_string($color))
      return Thumbnail::log('色碼格式錯誤，目前只支援字串 HEX 格式！', '色碼：' . json_encode($color));
  ...
  public function adaptiveResizePercent($width, $height, $percent) {
    ...
    if ($percent < 0 || $percent > 100)
      return Thumbnail::log('百分比例錯誤！', '百分比要在 0 ~ 100 之間！', '百分比：' . $percent);
  ...
  public function resizePercent($percent = 0) {
    if ($percent < 1)
      return Thumbnail::log('縮圖比例錯誤！', '百分比要大於 1', '百分比：' . $percent);
  ...
  public function crop($startX, $startY, $width, $height) {
    ...
    if ($startX < 0 || $startY < 0)
      return Thumbnail::log('起始點錯誤！', '水平、垂直的起始點一定要大於 0', '水平點：' . $startX, '垂直點：' . $startY);
  ...
  public function rotate($degree, $color = 'transparent') {
    if (!is_numeric($degree))
      return Thumbnail::log('角度一定要是數字！', '角度：' . $degree);
  ...
  public function filter($radius, $sigma, $channel = Imagick::CHANNEL_DEFAULT) {
    ...
    if (!is_numeric($radius))
      return Thumbnail::log('參數錯誤！', '參數 Radius 要為數字！', 'Radius：' . $radius);

    if (!is_numeric($sigma))
      return Thumbnail::log('參數錯誤！', '參數 Sigma 要為數字！', 'Sigma：' . $sigma);

    if (!in_array($channel, $items))
      return Thumbnail::log('參數錯誤！', '參數 Channel 格式不正確！', 'Channel：' . $channel);
  ...
  public function getAnalysisDatas($limit = 10) {
    if ($limit <= 0) {
      Thumbnail::log('參數錯誤！', '分析數量一定要大於 0', '分析數量：' . $limit);
      return [];
    }
  ...
  public function saveAnalysisChart($savePath, $font, $limit = 10, $fontSize = 14, $rawData = true) {
    if (!$savePath)
      return Thumbnail::log('錯誤的儲存路徑！', '儲存路徑：' . $savePath);

    if (!is_readable($font))
      return Thumbnail::log('參數錯誤！', '字型檔案不存在或不可讀！', '字型：' . $font);

    if ($limit <= 0)
        Thumbnail::log('參數錯誤！', '分析數量一定要大於 0', '分析數量：' . $limit);

    if ($fontSize <= 0)
      return Thumbnail::log('參數錯誤！', '字體大小一定要大於 0', '字體大小：' . $fontSize);

    $format = pathinfo($savePath, PATHINFO_EXTENSION);
    if (!$format || !in_array($format, self::allows()))
      return Thumbnail::log('不支援此檔案格式！', '格式：' . $format);

    if (!$datas = $this->getAnalysisDatas($limit))
      return Thumbnail::log('圖像分析錯誤！');
  ...
  public function addFont($text, $font, $startX = 0, $startY = 12, $color = 'black', $fontSize = 12, $alpha = 1, $degree = 0) {
    if (!$text)
      return Thumbnail::log('沒有文字！', '內容：' . $text);
    ...
    if ($alpha < 0 || $alpha > 1)
      return Thumbnail::log('參數錯誤！', '參數 Alpha 一定要是 0 ~ 1', 'Alpha：' . $alpha);
