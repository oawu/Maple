...
class S3 {
  ...
  public static function instance($accessKey, $secretKey, $isUseSsl = false, $isVerifyPeer = true) {
    $key = md5($accessKey . $secretKey . ($isUseSsl ? 1 : 0) . ($isVerifyPeer ? 1 : 0));
    
    if (isset(self::$instances[$key]))
      return self::$instances[$key];
    
    return self::$instances[$key] = new S3($accessKey, $secretKey, $isUseSsl, $isVerifyPeer);
  }
