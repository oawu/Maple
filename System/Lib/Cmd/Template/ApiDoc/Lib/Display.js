...
class Display {
  ...
  static line(title, error) {
    if (typeof title === 'string') {
      ...
      Display.lines = args.map(function(t, i) { return '\x1b[K' + ' '.repeat(3 + (i ? 2 : i * 2)) + (i ? Display.markHash() : Display.markList()) + ' ' + t.replace(/(^\s*)/g,''); });
      print(Display.lines.join(Display.LN) + Xterm.color.black('…', true) + ' ');
    }
  ...
  static error(errors) {
    if (typeof errors === 'undefined') process.exit(1);
    if (!Array.isArray(errors)) errors = [errors];
    if (!errors.length) process.exit(1);
    print(Display.LN + ' ' + Xterm.color.red('【錯誤訊息】') + Display.LN);
    print(errors.map(function(error) { return ' '.repeat(3) + Display.markList() + ' ' + error + Display.LN; }).join('') + Display.LN);
    process.exit(1);
  }
