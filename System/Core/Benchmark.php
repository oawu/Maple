...
class Benchmark {
  ...
  public static function time($key = null, $decimals = 6) {
    if ($key === null) {
      $arr = [];
    ...
    if (!isset(self::$times[$key]['s']))
      return null;

    isset(self::$times[$key]['e']) || self::$times[$key]['e'] = microtime(true);

    return number_format(self::$times[$key]['e'] - self::$times[$key]['s'], $decimals) . ' 秒';
  }

  public static function memory($key = null, $decimals = 6) {
    if ($key === null) {
      $arr = [];
    ...
    if (!isset(self::$memories[$key]['s']))
      return null;

    isset(self::$memories[$key]['e'])
      || self::$memories[$key]['e'] = memory_get_usage();
...
Status::addFuncs('Benchmark stop', function() {
  return Log::benchmark('耗時：' . Benchmark::time('整體'), '記憶體：' . Benchmark::memory('整體'));
});