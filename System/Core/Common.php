    ...
    foreach ($pieces as $piec)
      $ret .= isset($piec)
        ? !is_object($piec)
          ? !is_bool($piec)
            ? is_array($piec)
              ? '[' . implodeRecursive($glue, $piec) . ']' . $glue
              : $piec . $glue
...
if (!function_exists('isAssoc')) { // 判斷陣列是否為非正常正列 ['a', 'b'] => false, ['a' => 1, 'b' => 'c'] => true
  function isAssoc(array $array) {
    return array_keys($keys = array_keys($array)) !== $keys || !count($keys);
  }
}

if (!function_exists('dump')) { // 專門印變數用的(不錯用，可以印出 Model 之類的)
  function dump($val, $ln = "\n", $spaceStr = ' ', $level = 0) {
    ...
    if (is_array($val))
      if (isAssoc($val))
        return $space . '[' . (($tmp = implode(', ' . $ln, array_map(function ($k, $v) use ($level, $ln, $spaceStr) { return dump($k, $ln, $spaceStr, $level + 1) . ': ' . ltrim(dump($v, $ln, $spaceStr, $level + 1), $spaceStr); }, array_keys($val), $val))) ? $ln . $tmp . $ln . $space : '') . ']';
      else
        return $space . '[' . (($tmp = implode(',' . $ln, array_map(function($v) use ($level, $ln, $spaceStr) { return dump($v, $ln, $spaceStr, $level + 1); }, $val))) ?  $ln . $tmp . $ln . $space : '' ) . ']';

    if ($val instanceof \M\Model)
      return $space . 'Model(' . deNamespace(get_class($val)) . ') {' . $ln
            . implode(',' . $ln, array_map(function ($k, $v) use ($spaceStr, $level, $ln, $base) { return str_repeat($spaceStr, ($level + 1) * $base) . dump($k, $ln, $spaceStr, 0) . ': ' . ltrim(dump($v, $ln, $spaceStr, $level + 1), $spaceStr);}, array_keys($val->attrs()), $val->attrs())) . ($ln ? $ln . $space : '') . '}';

    if ($val instanceof \M\Core\Plugin\DateTime)
      return $space . ucfirst($val->type()) . '(' . ($val->format() ?? 'null') . ')';

    if ($val instanceof \M\Core\Plugin\Uploader) {
      $space2 = str_repeat($spaceStr, ($level + 1) * $base);
...
if (!function_exists('dirNotExistCreate')) {
  function dirNotExistCreate($path, $mode = 0777, $recursive = false) {
    return is_dir($path)
      ? true
      : @umaskMkdir($path, $mode, $recursive);
  }
}
...
if (!function_exists('benchmark')) {
  function benchmark($key = null) {
    static $benchmark = [];

    $format = function($tmp) {
      return [
        'time' => microtime(true) - $tmp['t'],
        'memory' => round((memory_get_usage() - $tmp['m']) / pow(1024, 2), 4)
      ];
    };

    return $key !== null
      ? !isset($benchmark[$key])
        ? !!$benchmark[$key] = ['t' => microtime(true), 'm' => memory_get_usage()]
        : $format($benchmark[$key])
      : array_map($format, $benchmark);
  }
}
